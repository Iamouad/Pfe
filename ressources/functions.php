...
function liste_pfe($filiere){
	...
	//$req = $bdd->prepare('SELECT * FROM ensas_pfe WHERE filiere = :filiere  ORDER BY groupe ');
	$req = $bdd->prepare('SELECT * FROM ensas_pfe ensas_pfe_options WHERE filiere = :filiere ORDER BY groupe ');
...
function get_emails_of_groupe($groupe){
    $dataBase = new DataBase();
    $bdd=$dataBase->connexionBdd();
    $req = $bdd->prepare('SELECT email1,email2,email3 FROM ensas_pfe WHERE groupe = :groupe  ');
    $req->execute(array(
