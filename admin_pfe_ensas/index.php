...
if(isset($_SESSION['user']))
    header('location:dsFolder/ProfileAdmin.php');
/*if($_SERVER["HTTPS"] != "on") {
   header("Location: https://fcensas.com/pfe/admin_pfe_ensas");
}*/

include "../db_connect.php";
require "dsFolder/DataBaseManager.php";
...
if(isset($_POST['submit'])) {
    ...
    if(isset($pass) && isset($user) ) {
        $query = $con->prepare('Select * from admin limit 1');
        $query->execute();
        $data = $query->fetch();
        if(($user == $data['login']) && ($pass == $data['password'])) {
            $_SESSION['user'] = $user ;
            $_SESSION['pass'] = $pass ;
            header('Location:dsFolder/profileAdmin.php');
        }else{
            ...
            if($data=$connection->fetch()){
                ...
                header('Location:dsFolder/profilProf.php');
            }else{
