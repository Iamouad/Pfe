...
if(isset($_POST['selected_choice']) and isset($_POST['selected_ordre']) ) {
    ...
    switch ($_POST['selected_choice']) {
        ...
        case 1:
            $req = $bdd->query("SELECT * FROM ensas_pfe  ORDER BY  " . $ordre."");
            break;
        case 2:
            $req = $bdd->query("SELECT * FROM ensas_pfe WHERE filiere='F' ORDER BY " . $ordre);
            break;
        case 3:
            $req = $bdd->query("SELECT * FROM ensas_pfe WHERE filiere='T'  ORDER BY " . $ordre);
            break;
        case 4:
            $req = $bdd->query("SELECT * FROM ensas_pfe  WHERE preembauche='oui'  ORDER BY " . $ordre);
            break;
        case 5:
            $req = $bdd->query("SELECT * FROM ensas_pfe  WHERE preembauche='non'  ORDER BY " . $ordre);
            break;
    }

}else{
    $req = $bdd->query("SELECT * FROM ensas_pfe ORDER BY filiere" );
}
?>
